...
export class CrashComponent {
     ...
     private hitBarrier(object: ObjectComponent) {
        ...
        if (this.leavesWith(object)) {
            switch (this.leavesWith(object)) {
                case 'right': object.movement.xPos = object.game.canvas.width - object.design.width; object.movement.speedX = 0; break;
                case 'left': object.movement.xPos = 0; break;
                case 'bottom': object.movement.yPos = object.game.canvas.height - object.design.height; object.movement.speedY = 0;
                    object.movement.speedY = 0;
                    // this.gravity = 0;
                    break;
                case 'top': object.movement.yPos = 0; break;
            }
            return true;
        }
        if (object.game.barriers.items.length > 0) {
            object.game.barriers.items.forEach(obj => {
                if (this.crashWith(object, obj)) {
                    object.movement.yPos = obj.movement.yPos - object.design.height;
                    return true;
                }
            })
        }
    ...
    crashWith(currentObj: ObjectComponent, otherobj): boolean {
        let myleft = currentObj.movement.xPos;
        let myright = currentObj.movement.xPos + (currentObj.design.width);
        let mytop = currentObj.movement.yPos;
        let mybottom = currentObj.movement.yPos + (currentObj.design.height);
    ...
    leavesWith(object: ObjectComponent): any {
        let myleft = object.movement.xPos;
        let myright = object.movement.xPos + (object.design.width);
        let mytop = object.movement.yPos;
        let mybottom = object.movement.yPos + (object.design.height);
        let otherleft = 0;
